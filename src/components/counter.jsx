...
class Counter extends Component {
    state = { 
        ...
        items: ['dresses', "shoes", "gowns", "tops"]
    ...
    render() { 
        ...
        return (
            <div>
            {this.state.items.length ===0 && 'Go to homage and start shopping!'}
            {this.renderList()}
            <span className={this.updateClasses()} style= {{fontSize: 20}}>{this.formatCount()}</span>
            <button className="btn btn-secondary btn-md" onClick= {(product) => this.handleIncrement(product)}>Increment</button>
